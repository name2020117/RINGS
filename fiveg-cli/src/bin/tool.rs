...
use fiveg_cli::schema::{policy_am_doc, policy_sm_doc, subscription_auth_doc, subscription_provision_am_doc, subscription_provision_sm_doc_1, subscription_provision_sm_doc_2, subscription_provision_smf_doc};
use mongodb::sync::{Client, Collection};

const POLICY_UE_AM: &str = "policyData.ues.amData";
const POLICY_UE_SM: &str = "policyData.ues.smData";
const SUB_AUTH: &str = "subscriptionData.authenticationData.authenticationSubscription";
...
fn main() -> Result<()> {
    ...
    let subscription_auth_collection: Collection<Document> = db.collection(SUB_AUTH);
    ...
    let mut subscription_auth_vec: Vec<Document> = Vec::new();
    ...
    subscription_auth_collection.insert_many(subscription_auth_vec, None)?;
