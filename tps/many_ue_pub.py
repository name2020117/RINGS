...
# make sure nr-binder is actually executable
# subprocess.run("chmod u+x ./build/nr-binder", text=True, check=True)
...
for i in range(1, args.n_ue + 1):
    ...
    subprocess.run(
        ...
        + f" -p 9092 -t test -n {args.m_messages} -s 1 -i uesimtun{i-1} &",
        text=True,
        shell=True,
    )
